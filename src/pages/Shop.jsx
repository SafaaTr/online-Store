...
function Shop() {
  ...
  const { searchValue } = useOutletContext(); // مررتا ضمن outlet لانو زر البحث بnavbar 
  ...
  useEffect(()=>{
    ...
    .then(
      res=>{
        setProducts(res.data) // axios بيرجع البيانات داخل res.data
        setLoading(false)
      })
    .then(err=>{
  ...
  .filter(item=> /// هون عملت الفلترة على كل المنتجات 
  ...
  return (
    ...
    <div className='mt-120 md:mt-170 md:mx-50 px-20 mb-20 md:mb-60 '>
      ...
      <div className=' md:flex justify-between gap-10  items-start '>
        ...
        <div className='md:grid flex-wrap justify-between lg:grid-cols-3 gap-12 mt-20'>
